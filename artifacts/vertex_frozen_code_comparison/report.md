...
|---|---:|---:|---:|---:|---:|---:|---:|
| sync | 0.8047 | 1.000 | 0.237 | 1.000 | 0.000 | 0.254 | 0.0391 |
| naive_async | 0.8773 | 1.090 | 0.303 | 1.281 | 0.176 | 0.102 | 0.0217 |
| stale_filter | 0.8991 | 1.117 | 0.310 | 1.308 | 0.176 | 0.102 | 0.0217 |
| random_filter | 0.8773 | 1.090 | 0.303 | 1.281 | 0.176 | 0.102 | 0.0217 |

## Vertex usage

The run used model version **gemini-2.5-flash-lite** and made **142** uncached Vertex requests (42506 input tokens and 5596 output tokens). The local conservative estimate was **$1.42** against the configured **$180.00** guard. This estimate is an experiment guard, not a Cloud Billing invoice.

## Wall-clock profile

Mean per-run wall time was **17.873s**: sampling/inference 17.820s, local policy updates 0.000s, and verification 0.022s. Raw profiles are stored beside each run summary.
...
```bash
python3 -m staleroll.cli run --policy vertex_frozen --task-domain code --seeds 2 --tasks 50 --workers 2 --ticks 64 --batch-size 4 --max-delay 8 --max-lag 0 --max-kl 0.2 --lag-decay 0.12 --kl-decay 0.4 --vertex-max-requests 80 --vertex-budget-usd 180.0 --output artifacts/gcp_code_benchmark_tuned
```
